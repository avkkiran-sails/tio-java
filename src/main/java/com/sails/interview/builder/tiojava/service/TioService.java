package com.sails.interview.builder.tiojava.service;

import com.sails.interview.builder.tiojava.model.TioRequest;
import com.sails.interview.builder.tiojava.model.TioResponse;
...
public class TioService {
     ...
     */
    public static final TioService MAIN;

    static {
        try {
            MAIN = new TioService(new URI("https://tio.run/"));
     ...
     *
     * @param root The root of this Tio instance. For example {@code https://tio.run/} is the root location of the {@link TioService#MAIN} server.
     * @throws MalformedURLException If the given URI is not a URL.
     */
    public TioService(URI root) throws MalformedURLException {
    ...
    public Supplier<TioResponse<Set<String>>> queryLanguages() {
        return () -> {
            try {
                ...
                object.keys().forEachRemaining(key -> keySet.add(key.toString()));
                return new TioResponse<>(connection.getResponseCode(), Optional.of(keySet), Optional.empty());
